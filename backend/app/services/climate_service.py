...
class ClimateService:
    ...
    def _build_filter_kwargs(
        ...
        """Parse and assemble filter kwargs once — shared by count and fetch."""
        return {
            "region": region,
    ...
    async def list_records(
        ...
        kwargs = self._build_filter_kwargs(
            region, source_type, from_date, to_date, is_anomaly, is_archived
        )
        items = await self.repo.list_paginated(page, limit, **kwargs)
    ...
    async def export_csv(
        ...
        # Parse once — count and fetch use identical filter
        kwargs = self._build_filter_kwargs(
            region, source_type, from_date, to_date, is_anomaly, is_archived
        )
        total = await self.repo.count_filtered(**kwargs)
        ...
        records = await self.repo.list_for_export(EXPORT_CAP, **kwargs)
